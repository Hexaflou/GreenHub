...
void *thread_init_func(void *arg)
{
	...
	char msgflag[]="Message recu Mr Bond";
	char msgend[]="\r\n Fin vague Server \r\n";
	char c;
	int n;
#ifdef DEBUG
	...
	while(1)
	{
		...
		for(i = 0; i < MAX_CLIENT_PER_THREAD; i++)
		{
			...
			{
				//printf("Server Waiting for Data from Clients \n");
				...
				else if(n < 0)
				{
					...
					send(threads[tid].clients[i], msgflag, strlen(msgflag), 0);
					memset((char *) &buffer, 0, strlen(buffer));
				}
			}
		}
		...
		//send(threads[0].clients[0], msgend, strlen(msgend), 0);
